...
def build_ngram_probability_matrix(corpus,
                                   freq_dist_file,
                                   matrix_filename,
                                   num_words=10000,
                                   stopwords=[]):
    """
    Generate a DS matrix containing bigram probabilities.

    Parameters
    ----------
    corpus : str
        Filename containing a pre-tokenized corpus.
    freq_dist_files : str
        Filename of a file containing frequencies for the same corpus.
        I.e. every line holds a number of occurences
        and the word to which that number belongs,
        such that the most common words are at the beginning.
        Note that you don't have to follow use order, as long as the
        file follows the "number word" format.
        In order to obtain the frequencies,
        a fast way is to run the follwing terminal command
        (assuming that the tokens are separated by
        a spacing character like \t or space):
        tr "\t" "\n" < corpus_file | sort | uniq -c | sort -nr > frequency_file
    matrix_filename : str
        Filename to which to save the generated matrix.
    num_words : int
        Number of words to use in the matrix. Is capped to 100000.
    stopwords : [str]
        Words to exclude from matrix.
    """
    ...
    # Note: here a very bizarre things happens
    # if I don't restrict the number of words:
    # some of the values are randomly reduced
    ...
    print("number of types (including START$_ and END$_ ):",
          len(vocab_order), "\n")
    ...
    # the shape of the matrix of it is equal to the brigram lexicon
    shape = (len(freq_dict), len(freq_dict))
    matrix = scipy.sparse.lil_matrix(shape, dtype=np.float64)
    ...
    for sent in open(corpus):
        ...
        bigrams = [(current_sent[i], current_sent[i+1])
                   for i in range(len(current_sent)-1)]

        # if both words in the bigram are in the lexicon
        # we add the count to the matrix
        for i in bigrams:
...
def read_predict_vectors(predict_filename,
                         matrix_directory,
                         prev_matrix_name=None,
                         word_num=None):
    """
    Read predict vectors from
    ...
    prev_matrix_name : str
         ...
         Use to have this matrix have the same vocabulary
         and word order as the old one.
    word_num : int
    ...
    with open(predict_filename) as predict_file:
        for i, line in enumerate(predict_file):
            ...
            if old_vocab_order is not None:
                if word not in old_vocab_order:
                    continue
